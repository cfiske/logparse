    ...
    while True:
        try:
            ...
            if int(datetime.utcnow().strftime("%S")) != ssec:
                ...
                eprint("%28s Messages in buffer (peak): %d read: %d processed: %d yielded: %d skipped: %d" % (str(datetime.utcnow()), pktbuf_peak, packets, processed, yieldcount, skipcount))
                pktbuf_peak = 0
